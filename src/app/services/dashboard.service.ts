...
export class DashboardService {
    ...
    // Update json
    updateDashboard(id: string, params): Observable<any> {
        const httpOptions = { headers: new HttpHeaders({ 'Content-Type':  'application/json' })};
        return this._http.put<any>(`${this.url}/store/${id}`, params, httpOptions);
    }

    // Dashboard node
    getDashboardNode(id: string): Observable<any> {
        return this._http.get<any>(`${this.url}/node/${id}`);
    }
    ...
    // set Dashboard store
    setDashboardStore(id: string, config: object): Observable<any> {
        return this._http.post<any>(`${this.url}/store/`, config);
    }

    // Dashboard menu
    getDashboardMenu(): Observable<any> {
        return this._http.get<any>(`${this.url}/menu/`);
    }
