...
export class TabFlowComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    initData() {
        ...
        this.aliasTitle = Object.keys(hosts).map( i => {
            const __alias = Object.entries(this.dataItem.data.alias).find(j => (j[0].startsWith(i+':') || j[0].startsWith('['+i+']'+':')));
        ...
        if (at.length === 2 && at[1].arrip.length > 1 && this._isCombineByAlias) {
            at.push({
                empty: true
            });
        }
